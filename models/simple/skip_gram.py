...
class SkipGram(ModelBase):
    ...
    def forward(self, input_batch, **kwargs):
        ...
        # computing out loss
        emb_input = self._embeddings_input.forward(input_words)     # bs, emb_dim
        emb_context = self._embeddings_context.forward(context_words)  # bs, emb_dim
        ...
        num_neg_samples_for_this_batch = context_words.shape[0]*self._negative_samples
        ...
        negative_example = negative_example.view(context_words.shape[0], self._negative_samples).to(self._arguments_service.device) # bs, num_neg_samples

        emb_negative = self._embeddings_context.forward(negative_example) # bs, neg_samples, emb_dim
    ...
    def get_embeddings(self, tokens: torch.Tensor) -> torch.Tensor:
        embeddings = self._embeddings_input.forward(tokens)
